...
namespace _6_3
{
    internal class Program
    {
        ...
        static void Input()
        {
            if (!File.Exists(@"e:\rabs.txt"))
            {
                File.Create(@"e:\rabs.txt").Close();
                Console.WriteLine("Файл создан");
            }
            using (StreamWriter sw = new StreamWriter("rabs.txt", true, Encoding.Unicode))
            {
                char Key;
                do
                {
        ...
        static void Print()
        {
            using (StreamReader sr = new StreamReader(@"e:\rabs.txt", Encoding.Unicode))
            {
                string line;
